-- CATALYST EDUCATION - SUPABASE SCHEMA
...
CREATE POLICY "Teachers can view students in their classes" ON public.students FOR SELECT USING (class_id IN (SELECT id FROM public.classes WHERE teacher_id = auth.uid()));
CREATE POLICY "Teachers can update students in their classes" ON public.students FOR UPDATE USING (class_id IN (SELECT id FROM public.classes WHERE teacher_id = auth.uid()));
CREATE POLICY "Teachers can add students to their classes" ON public.students FOR INSERT WITH CHECK (class_id IN (SELECT id FROM public.classes WHERE teacher_id = auth.uid()));

CREATE POLICY "Teachers can manage their lesson plans" ON public.lesson_plans FOR ALL USING (teacher_id = auth.uid());

CREATE POLICY "Teachers can manage their assessments" ON public.assessments FOR ALL USING (teacher_id = auth.uid());

CREATE POLICY "Teachers can manage questions for their assessments" ON public.assessment_questions FOR ALL USING (assessment_id IN (SELECT id FROM public.assessments WHERE teacher_id = auth.uid()));

CREATE POLICY "Teachers can manage attendance for their classes" ON public.attendance FOR ALL USING (class_id IN (SELECT id FROM public.classes WHERE teacher_id = auth.uid()));
...
-- Initial data
INSERT INTO public.schools (id, name, school_type, region) VALUES (uuid_generate_v4(), 'Catalyst Demo School', 'secondary', 'Lusaka') ON CONFLICT DO NOTHING;
